...
def search():
    ...
    if query:
        ...
        for i in range(230000):
            ...
            if research == "char" and (
                query == chr(i)
                or query.upper() in unicodedata.name(chr(i), "Name Not Found")
            ):
                results.append(
                    {
                        "display": chr(i),
            ...
            if (
                research == "code"
                and (
                    unquote(query) == "U+%04x" % ord(chr(i))
                    or unquote(query) == "u+%04x" % ord(chr(i))
                )
                and i not in range(55296, 57343)
            ):
                results.append(
                    {
                        "display": chr(i),
            ...
            if research == "name" and query.upper() in unicodedata.name(
                chr(i), "Name Not Found"
            ):
                results.append(
                    {
                        "display": chr(i),
